...
public class arrowTeamSelector2 : MonoBehaviour {
...
public GameObject gizmoRT;
	...
	Debug.Log("run move teams select gizmo");
		...
		if (totVoltThisPeriodRight < -totVoltTriggerMove) {
			...
			if(Mathf.Abs(gizmoRT.transform.position.x)> Mathf.Abs(gizmoSittingPositions[0].transform.position.x) ){
			int indexForMostRightPositionLeftOfMe = -1;
			float currentSmallestPositionalDifference = 10000000f; 
			for(int i = 0; i<3; i++){
					if(Mathf.Abs(gizmoSittingPositions[i].transform.position.x) <  Mathf.Abs(gizmoRT.transform.position.x) && Mathf.Abs(gizmoRT.transform.position.x-gizmoSittingPositions[i].transform.position.x )<currentSmallestPositionalDifference){
			...
			Debug.Log("should move right");
				if(Mathf.Abs(gizmoRT.transform.position.x)< Mathf.Abs(gizmoSittingPositions[3].transform.position.x )){
					Debug.Log("Im not in the most right position");
					int indexForMostLeftPositionRightOfMe = -1;
					float currentSmallestPositionalDifference = 10000000f; 
							for(int i = 0; i<4; i++){
						Debug.Log("i is " +i+ " this if statement is " + (Mathf.Abs(gizmoSittingPositions[i].transform.position.x)  > Mathf.Abs(gizmoRT.transform.position.x) && Mathf.Abs(gizmoRT.transform.position.x -gizmoSittingPositions[i].transform.position.x )<currentSmallestPositionalDifference) );
						if(Mathf.Abs(gizmoSittingPositions[i].transform.position.x)  > Mathf.Abs(gizmoRT.transform.position.x) && Mathf.Abs(gizmoRT.transform.position.x -gizmoSittingPositions[i].transform.position.x )<currentSmallestPositionalDifference){
							currentSmallestPositionalDifference = Mathf.Abs(gizmoRT.transform.position.x -gizmoSittingPositions[i].transform.position.x);
