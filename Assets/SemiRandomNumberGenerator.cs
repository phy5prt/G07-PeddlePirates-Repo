...
private float	NAddedRangeHigh;
	...
	float rangeBell = 0;
		for(int i = 0; i <roundnessBellCurveInRNAddedRange; i++){
			rangeBell += Random.Range (rNAddedRangeLow, NAddedRangeHigh) /roundnessBellCurveInRNAddedRange ;
		}
	...
	{
		rNAddedRangeLow = rNAddedRange + directionBiasOfRn * rNAddedRange;
		NAddedRangeHigh = -rNAddedRange + directionBiasOfRn * rNAddedRange;
		rNAdded = makeRangeBell ();
		theRandomNumber = theRandomNumber + rNAddedRange * Time.deltaTime * sensitivityToRateOfUpdate;
